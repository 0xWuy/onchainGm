...
import { createWalletClient, custom, numberToHex, parseGwei, type Hash } from "viem";
...
export function App() {
  ...
  const selectedChainId = useMemo(() => {
    if (getGmContractAddress(activeChainId)) {
  ...
  const isBusy = txState === "wallet" || txState === "confirming" || isSwitching;
    ...
    <main className="app-shell">
      ...
      <section className="hero-band">
        <div className="hero-copy">
          ...
          <p>
            A compact starter app for learning wallet connection, contract reads,
            and contract writes on Arc.
          </p>
        </div>
      ...
      <section className="stats-grid" aria-label="GM stats">
        ...
        />
        <StatCard icon={<ShieldCheck size={18} />} label="Network" value={selectedChain.name} />
      </section>
      ...
      <section className="workspace">
        <div className="composer-panel">
          <div className="panel-heading">
            ...
            <StatusPill selectedChainName={selectedChain.name} isWrongChain={isWrongChain} />
          </div>
          ...
          <div className="contract-row">
            <div>
              ...
              <strong>{gmAddress ? shortAddress(gmAddress) : "Not deployed yet"}</strong>
            </div>
            {gmAddress ? (
              <button className="icon-button" type="button" onClick={copyContractAddress}>
          ...
          <div className="feed-list">
            {isLoading ? (
              ...
              recentGms.map((gm, index) => (
                <article className="feed-item" key={`${gm.sender}-${gm.createdAt}-${index}`}>
                  <div className="feed-avatar">{gm.message.slice(0, 2).toUpperCase()}</div>
                  <div>
    ...
    <header className="topbar">
      ...
      {isConnected ? (
        ...
        <button className="wallet-button" type="button" onClick={onConnect}>
          {isConnecting ? <Loader2 className="spin" size={17} /> : <Wallet size={17} />}
          Connect
        </button>
      )}
    </header>
